...
export default function OrbitImages({
  ...
  const items = images.map((src, index) => (
    <img
      ...
      draggable={false}
      className={`h-full w-full object-contain ${circular ? "rounded-full border-2 border-white/20 bg-black/80 shadow-[0_0_12px_rgba(255,255,255,0.15)]" : ""}`}
    />
  ))
  ...
  return (
    ...
    >
      <div
        className={
          responsive ? "absolute top-1/2 left-1/2" : "relative h-full w-full"
        }
        style={{
